...
function LoginUi:init_widget()
    ...
    self.startgame_object = self.uiRoot.transform:Find("ConfirmLogin").gameObject
    self.server_object = self.uiRoot.transform:Find("Server").gameObject
    
    print("<color=orange>"..self.server_object.name.."</color>")
    ...
    --ML_GameMaster.mono:AddClick(self.startgame_object, self.player_submit, self)
    --ML_GameMaster.add_click()
    print(type(self.player_submit));
    ML_GameMaster.mono:AddClick(self.server_object , self.login_submit, self)
end 
...
function LoginUi:login_submit(click)
